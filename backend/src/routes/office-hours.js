...
router.patch("/book/:slotId", async (req, res) => {
  try {
    ...
    // Notify the lecturer that someone booked their slot
    const Notification = require("../models/Notification");
    await new Notification({
      userId: slot.lecturerId,
      title: "Office Hour Booked",
      message: `A student booked your ${slot.dayOfWeek} ${slot.startTime} slot`,
      type: "office_hour_booked",
      relatedId: slot._id,
      dedupeKey: `oh_booked_${slot._id}_${Date.now()}`,
    })
      .save()
