  ...
  try {
    ...
    if (workId === "new") {
      if (!groupId || !assignmentId) {
        return res
          .status(400)
          .json({ message: "Missing groupId or assignmentId" });
      }
    ...
    if (!member || member.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only group admins can manage tasks" });
    }
    ...
    // After saving the new subtask, notify the assigned member
    if (assignedTo) {
      ...
      const notification = new Notification({
        ...
        dedupeKey: `task_assigned_${work._id}_${Date.now()}`,
      });
      await notification
        .save()
...
router.patch("/:workId/subtasks/:subTaskId/complete", async (req, res) => {
  try {
    ...
    res.json(updatedWork);
    // Notify group admins that a task needs approval
    const group = await StudyGroup.findById(work.groupId);
    ...
    for (const admin of admins) {
      await new Notification({
        ...
        message: `A task "${task.title}" is pending your approval`,
        type: "task_pending_approval",
        relatedId: work.groupId,
        dedupeKey: `task_pending_${task._id}_${admin.userId}_${Date.now()}`,
      })
        .save()
...
router.patch("/:workId/subtasks/:subTaskId/approve", async (req, res) => {
  ...
  try {
    ...
    res.json(updatedWork);
    // Notify the member who completed the task
    if (task.completedBy) {
      const Notification = require("../models/Notification");
      await new Notification({
        ...
        type: "task_approved",
        relatedId: work.groupId,
        dedupeKey: `task_approved_${task._id}_${Date.now()}`,
      })
        .save()
        .catch((e) => console.error(e));
